...
def get_rule_from_ddos_protector(ddp_rule_id, base_url, api_key, key_header='x-api-key') -> Optional[Response]:
    """Remove a DDoS Protector rule from a DDoS Protector device using DDoS Protector's REST API
    If the app is in testing mode, returns None and does not send anything.
    ...
    """
    if not current_app.config['TESTING']:
        result = requests.get(base_url + '/rules/' + str(ddp_rule_id), headers={key_header: api_key})
        return result
    return None
